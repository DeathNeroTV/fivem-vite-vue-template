<script setup lang="ts">
	...
	const pageOptions = [5, 10, 25, 50, 100].map((v) => ({
		label: String(v),
		value: v,
	}));
	...
	function getValue(obj: any, path?: string, accessorFn?: Function) {
		if (accessorFn) return accessorFn(obj);
	...
	const filteredData = computed(() => {
		...
		return safeData.value.filter((row) => flatten(row).some((val) => String(val).toLowerCase().includes(term)));
	});
	...
	const sortedData = computed(() => {
		...
		return [...filteredData.value].sort((a, b) => {
			const valA = getValue(a, col?.key, col?.accessorFn);
			const valB = getValue(b, col?.key, col?.accessorFn);
	...
	<div class="relative flex flex-col w-full h-full select-none gap-5">
		...
		<div class="flex w-full items-center justify-between gap-2">
			...
			<div class="flex w-full items-center justify-end gap-2">
				<font-awesome-icon v-if="searchable" icon="magnifying-glass" class="h-10 w-10 p-2 text-2xl" />
				<input
					v-if="searchable"
					v-model="search"
					type="text"
		...
		<div class="flex w-full h-full flex-col rounded-xl relative overflow-hidden">
			...
			<!-- Body -->
			<div class="flex flex-col w-full h-full overflow-y-auto rounded-b-xl bg-neutral-800/60">
				<div
					v-for="row in paginatedData"
					:key="getValue(row, idKey) ?? JSON.stringify(row)"
					...
					<div v-for="col in columns" :key="col.key" class="flex-1 px-4 py-2 text-center">
						{{ col.format ? col.format(row) : getValue(row, col.key, col.accessorFn) }}
					</div>
